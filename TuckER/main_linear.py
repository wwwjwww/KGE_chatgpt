...
class Experiment:
    ...
    def evaluate(self, model, data):
        ...
        for i in range(0, len(test_data_idxs), self.batch_size):
            ...
            if self.cuda:
                ...
                r_idx = r_idx.cuda()
                e2_idx = e2_idx.cuda()
            predictions, matrix = model.forward(e1_idx, r_idx, triple_matrix)
    ...
    def train_and_eval(self):
        ...
        triple_embedding = torch.tensor(np.load("../../KGE_chatgpt/embed/triple_embedding_llama_avg.npy", allow_pickle=True))
        ...
        for it in range(1, self.num_iterations+1):
            ...
            for j in range(0, len(er_vocab_pairs), self.batch_size):
                ...
                if (pretrain_batch_values == 1).any():
                    ...
                    triple_matrix = torch.tensor(self.get_matrix(model.E, model.R, triple_idx))
                if self.cuda:
                    e1_idx = e1_idx.cuda()
                    r_idx = r_idx.cuda()
                    predictions, matrix = model.forward(e1_idx, r_idx, triple_matrix)
                ...
                if matrix != None:
                    ...
                    #np.save('./cmp/triple_embedding_%d.npy'%j, matrix.detach().cpu().numpy())
